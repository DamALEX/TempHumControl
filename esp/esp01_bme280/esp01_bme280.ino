...
void setup() {
  ...
  humidity = bme.readHumidity();          // Lecture de l'humidité en Pourcents
  Serial.print("Humidité: ");
  Serial.println(humidity);
  
  temp = bme.readTemperature();     // Lecture de la température en Celsius
  Serial.print("Temperature: ");
  Serial.println(temp);

  pressure = (bme.readPressure() / 100.0F);     // Lecture de la pression en Pascals
  Serial.print("Pression: ");
  Serial.println(pressure);

  pressure = (bme.readAltitude(SEALEVELPRESSURE_HPA));     // Lecture de l'altitudes Métres
...
void loop() {
  ...
  char strbufferpres[10];
  char strbufferalt[10];
  ...
  if (req.indexOf("/gettemp") != -1) {
    humidity = bme.readHumidity();          // Read humidity (percent)
  Serial.print("Humidité: ");
  Serial.println(humidity);
  
  temp = bme.readTemperature();     // Read temperature as Celsius
  Serial.print("Temperature: ");
  Serial.println(temp);

  pressure = (bme.readPressure() / 100.0F);     // Read pressure as Pascals
  Serial.print("Pression: ");
  Serial.println(pressure);

  pressure = (bme.readAltitude(SEALEVELPRESSURE_HPA));     // Read altitude as Métres
  Serial.print("Altitude: ");
  Serial.println(altitude);
  ...
  dtostrf(pressure, 4, 3, strbufferpres);
  dtostrf(altitude, 4, 3, strbufferalt);
