...
void setup() {
  ...
  if (!myHTU21D.begin(2, 0)) {
    Serial.println("Impossible de trouver un capteur HTU21D valide, vérifiez le câblage!");
    while (1);
  }
  
  humidity = myHTU21D.readHumidity();          // Lecture de l'humidité en Pourcents
  Serial.print("Humidité: ");
  Serial.println(humidity);
  
  temp = myHTU21D.readTemperature();     // Lecture de la température en Celsius
...
void loop() {
  ...
  temp = myHTU21D.readHumidity();     // Lecture de la température en Celsus
  Serial.print("Temperature: ");
  Serial.println(temp);
